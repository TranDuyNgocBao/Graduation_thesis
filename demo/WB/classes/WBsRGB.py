...
class WB:
  def __init__(self, gamut_mapping=2, upgraded=0, K=None, sigma=None):
    self.K = K
    self.sigma = sigma
    if upgraded == 1:
      self.features = np.load('WB/models/features+.npy')  # encoded features
      self.mappingFuncs = np.load('WB/models/mappingFuncs+.npy')  # correct funcs
      self.encoderWeights = np.load('WB/models/encoderWeights+.npy')  # PCA matrix
      self.encoderBias = np.load('WB/models/encoderBias+.npy')  # PCA bias
      if not K: self.K = 75  # K value for NN searching
    else:
      self.features = np.load('WB/models/features.npy')  # encoded features
      self.mappingFuncs = np.load('WB/models/mappingFuncs.npy')  # correction funcs
      self.encoderWeights = np.load('WB/models/encoderWeights.npy')  # PCA matrix
      self.encoderBias = np.load('WB/models/encoderBias.npy')  # PCA bias
      if not K: self.K = 25  # K value for nearest neighbor searching

    if not sigma: self.sigma = 0.25  # fall-off factor for KNN blending
